if SERVER
    ...
    combinations = (a, b, desiredW, desiredH) ->
        ...
        for _, one in pairs ones
            for _, zero in pairs zeroes
                ...
                if o and isPathSeamless o
                    isDupe = false
                    for k, v in pairs combs
                        if compareMatrices o, v
                            isDupe = true
                            break
                    if not isDupe
    ...
    duplicates = {}
    polyos = {}
    permute {polyo1, polyo2, polyo3}, 3, polyos

    --desiredW = 3
    --desiredH = 5

    for k, v in pairs polyos
        combs1 = (combinations v[1], v[2])
        for _, comb1 in pairs combs1
            combs2 = (combinations comb1, v[3], desiredH, desiredW)
            for _, comb2 in pairs combs2
                table.insert duplicates, comb2

    combs = {}
    for k, v in pairs duplicates
        isDupe = false
        for k, comb in pairs combs
            if compareMatrices v, comb
                isDupe = true
                break
        if not isDupe
