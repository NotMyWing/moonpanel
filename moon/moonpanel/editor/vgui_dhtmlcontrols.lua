...
local buildLink = "https://windmill.thefifthmatt.com/build"
local siteLink = "https://windmill.thefifthmatt.com"
local sitepattern = siteLink .. "/(.+)"
function PANEL:IsValidURL()
	local link = self.AddressBar:GetText() 
	if (string.find(link, buildLink) == 1) then
		return (link ~= buildLink) and (link ~= (buildLink .. "/"))
	else
		return (string.find(link, siteLink) == 1) and (link ~= siteLink) and (link ~= (siteLink .. "/"))
	end
end
...
function PANEL:Init()
	...
	local __panel = self
	self.ImportButton = vgui.Create( "DButton", self )
	self.ImportButton:SetSize( ButtonSize * 6, ButtonSize )
	self.ImportButton:Dock( RIGHT )
	self.ImportButton:DockMargin( Spacing, Margins * 3, Margins * 3, Margins * 3 )
	self.ImportButton:SetText("Import...")
	self.ImportButton:SetFont("DermaLarge")
	self.ImportButton:SetTextColor(Color(220, 220, 220))
	self.ImportButton.Paint = function (self, w, h)
		local color = Color(32, 32, 32)
		if __panel:IsValidURL() then
			local mod = (1 + math.cos(CurTime() * 10)) / 2 * 60
			color = Color(64 - mod, 200 - mod, 64 - mod)
		end

		draw.RoundedBoxEx( 2, 0, 0, w, h, color, true, true, true, true )
	end
	self.ImportButton.DoClick = function()
		if __panel.ImportCallback and __panel:IsValidURL() then
			local url = __panel.AddressBar:GetText()
			url = string.match(url, sitepattern)

			if url and url ~= "" then
				__panel.ImportCallback(url)
			end
		end
	end
	...
	self.HomeURL = "https://windmill.thefifthmatt.com/"
